...
import static com.teamscale.jacoco.agent.upload.teamscale.ETeamscaleServerProperties.REPOSITORY_ID;
...
public class TeamscaleUploader implements IUploader, IUploadRetry {
	...
	public void reupload(CoverageFile coverageFile, Properties reuploadProperties) {
		...
		server.repositoryId = Strings.emptyToNull(reuploadProperties.getProperty(REPOSITORY_ID.name()));
	...
	private Properties createServerProperties() {
		...
		serverProperties.setProperty(REPOSITORY_ID.name(), teamscaleServer.repositoryId);
	...
	private boolean tryUploading(CoverageFile coverageFile, TeamscaleServer teamscaleServer) {
		...
		try {
			...
			api.uploadReport(teamscaleServer.project, teamscaleServer.commit, teamscaleServer.revision,
					teamscaleServer.repositoryId, teamscaleServer.partition, EReportFormat.JACOCO,
