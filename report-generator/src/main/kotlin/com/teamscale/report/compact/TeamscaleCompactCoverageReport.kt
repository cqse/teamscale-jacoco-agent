 ...
 * @param fileCoverageInfos Coverage information on an aggregated per-file granularity.
 */
data class TeamscaleCompactCoverageReport @JsonCreator constructor(
	@JsonProperty("version") val version: Int,
	@JsonProperty("coverage") val fileCoverageInfos: List<CompactCoverageFileInfo>
) {
	/** Serializes the compact coverage into a JSON report and writes it into the given output stream. */
	fun writeTo(output: OutputStream) {
		val module = SimpleModule().apply {
			addSerializer(CompactLines::class.java, LineRangeSerializer())
		}
