...
class TeamscaleProfilerConfigurationSystemTest {
	...
	fun systemTestRetrieveConfig() {
		...
		profilerConfiguration.configurationOptions = """
			...
			""".trimIndent()
		val teamscaleMockServer = TeamscaleMockServer(FAKE_TEAMSCALE_PORT)
		...
		assertThat(teamscaleMockServer.uploadedReports).element(0)
			.extracting("partition").isEqualTo("foo")

		teamscaleMockServer.shutdown()

		assertThat(teamscaleMockServer.profilerEvents)
			.`as`("We expect a sequence of interactions with the mock. Note that unexpected interactions can be caused by old agent instances that have not been killed properly.")
			.containsSubsequence(
				"Profiler registered and requested configuration my-config",
				"Profiler 123 sent logs", // At least one log expected.
