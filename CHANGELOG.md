...
# Next Release
- [feature] Option `teamscale-revision-manifest-jar` which can be used to read the git commit revision from the `META-INF/MANIFEST.MF` in the Jar file. The field must be named `Revision` and must be provided as a main attribute or the field must be callsed `Git_Commit` and must be an attribute of an entry called `Git`.
