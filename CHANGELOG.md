...
# Next Release
- [feature] Option `teamscale-revision-manifest-jar` which can be used to read the git commit revision from the `META-INF/MANIFEST.MF` in the Jar file. The field must be named `Git_Commit` and must be provided as a main attribute or as an attribute of an entry called `Git`.
